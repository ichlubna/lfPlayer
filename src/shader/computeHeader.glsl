...
{
    vec4 c = texture(sampler2D(frameTexture(textID), textSampler), coord);
    mat4 block;
    for(int i=0; i<4; i++)
        block[i] = texture(sampler2D(frameTexture(textID), textSampler), coord+(vec2(1.0f,1.01f)-2*vec2(i/2, i%2))*LF_HALF_PX_SIZE*3);
    block[0][3] = c.r;
    block[1][3] = c.g;
    block[2][3] = c.b;
    return block;
